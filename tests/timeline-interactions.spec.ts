...
test('dated hover bands and independent event navigation',async({page})=>{
 ...
 expect(left).toBeCloseTo(38/192*100,1);
 ...
 expect(width).toContain(String(3/192*100).slice(0,5));
 await page.locator('.timeline-event-list button').filter({hasText:'Landing team designated for planning'}).click();
 await expect(page.getByRole('tab',{name:'Narrative',exact:true})).toHaveAttribute('aria-selected','false');
 await expect(page.getByRole('button',{name:'Full timeline',exact:true})).toHaveCount(0);
 await expect(page.locator('.timeline-event-list li')).toHaveCount(65);
 ...
 await expect(page.getByRole('region',{name:'Selected timeline event'})).toContainText('transferred');
 ...
 await expect(page.locator('.map-scopes button').filter({hasText:'Go to Camp Margarita'})).toHaveAttribute('aria-pressed','true');
});
