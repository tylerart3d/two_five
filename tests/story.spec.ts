...
test('story tabs separate narrative and Closer Look with keyboard and source navigation', async ({ page }) => {
  await page.goto('/#map');
  const narrative = page.getByRole('tab', { name: 'Narrative', exact: true });
  const chronology = page.getByRole('tab', { name: 'Closer Look', exact: true });
